...
// https://dic.pixiv.net/a/%E8%85%90%E5%90%91%E3%81%91%E3%83%98%E3%82%BF%E3%83%AA%E3%82%A2#h2_1
...
(() => {
  const characters = [
    ...
    {
      'short_country_name': '丁',
      'country_name': 'デンマーク',
      'name': 'マー君'
    },
    {
      'short_country_name': '諾',
      'country_name': 'ノルウェー',
      'name': 'ノル君'
    },
    {
      'short_country_name': '氷',
      'country_name': 'アイスランド',
      'name': '17億'
    },
    {
      'short_country_name': '蘭',
      'country_name': 'オランダ',
      'name': '蘭にいさん'
    },
    {
      'short_country_name': '白',
      'country_name': 'ベルギー',
      'name': 'ベルベル'
    },
    {
    ...
    {
      'short_country_name': '香',
      'country_name': '香港',
      'name': '大英帝国の呪い'
    },
    {'short_country_name': '澳', 'country_name': 'マカオ', 'name': '澳門さん'},
    {'short_country_name': '', 'country_name': 'チベット', 'name': 'チベさん'},
    {
      'short_country_name': '機',
      'country_name': 'キプロス',
      'name': 'キプくん'
    },
    {
  ...
  const getCouplingFromCharactersTable = title_tag => {
    ...
    const current_character =
        characters.find(x => x.name === current_character_name);
    ...
    const couplings =
        Array.from(target_table.querySelectorAll('tr'))
            ...
            .map(x => {
              ...
              const tag_name =
                  getDictTitleFromURL(x[1].querySelector('a').href);

              const getShortName = name => name.split('・').pop();
              return {
                characters: [
                  {
                    name: `${current_character.country_name}（${
                        getShortName(current_character.name)}）`,
                    dict_entry: current_character.name
                  },
                  {
                    name: `${character.country_name}（${
                        getShortName(character.name)}）`,
  ...
  const target_couplings =
      title_tags.map(x => getCouplingFromCharactersTable(x))
          .reduce((s, x) => [...s, ...x]);
